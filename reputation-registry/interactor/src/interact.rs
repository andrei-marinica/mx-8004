...
pub async fn reputation_registry_cli() {
    ...
    match cmd.as_str() {
        ...
        "submitFeedback" => interact.submit_feedback().await,
        "authorizeFeedback" => interact.authorize_feedback().await,
        "appendResponse" => interact.append_response().await,
        "getReputationScore" => interact.reputation_score().await,
        "getTotalJobs" => interact.total_jobs().await,
        "getValidationContractAddress" => interact.validation_contract_address().await,
        "getIdentityContractAddress" => interact.identity_contract_address().await,
        "hasGivenFeedback" => interact.has_given_feedback().await,
        "isFeedbackAuthorized" => interact.is_feedback_authorized().await,
        "getAgentResponse" => interact.agent_response().await,
        "setIdentityContractAddress" => interact.set_identity_contract_address().await,
        "setValidationContractAddress" => interact.set_validation_contract_address().await,
